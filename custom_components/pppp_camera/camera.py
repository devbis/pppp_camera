...
import httpx
from aiopppp import JsonSession
...
class PPPPCamera(Camera):
    ...
    async def async_camera_image(
        ...
        while not self._camera_session.info_requested:
    ...
    def on_device_disconnect(self, camera: aiopppp.types.Device):
        if self._camera_session:
            self._camera_session.main_task.cancel()
        if self._discovery:
            self._discovery.delete_device(camera)
    ...
    async def instantiate_session(self):
        ...
        if not self._camera_session:
            ...
            self._camera_session = JsonSession(self._camera_device, on_disconnect=self.on_device_disconnect)
